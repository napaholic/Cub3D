...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/05/13 17:42:09 by yeju              #+#    #+#             */
/*   Updated: 2022/05/13 17:46:43 by yeju             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (wall_data->side)
		wall_data->perp_wall_dist = (wall_data->map_pos_y - \
			info->pos->pos_y + (1 - wall_data->step_y) / 2) / \
				wall_data->raydir_y;
	else
		wall_data->perp_wall_dist = (wall_data->map_pos_x - \
			info->pos->pos_x + (1 - wall_data->step_x) / 2) / \
				wall_data->raydir_x;
	return ;
}
...
void	step_progress_until_hit(t_wall_data *wall_data, t_info *info)
{
	...
	{
		if (info->map->world_map[wall_data->map_pos_x] \
			[wall_data->map_pos_y] == '1')
		{
			wall_data->hit = 1;
		}
